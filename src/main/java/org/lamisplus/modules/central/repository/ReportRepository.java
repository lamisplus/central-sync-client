...
public interface ReportRepository extends JpaRepository<Report, Long> {
    ...
    @Query(value = "WITH bio AS (SELECT hc.client_code AS clientCode,      \n" +
            ...
            "\t\t\tWHERE hc.facility_id=?1 AND hc.date_visit >=?2 AND hc.date_visit <= ?3\n" +
            "),\n" +
            "ins AS (\n" +
            "SELECT DISTINCT hc.uuid,partner.noofpartnerselicited, hc.hiv_test_result, hc.client_code, hc.person_uuid, \n" +
            "\t(CASE WHEN hie.hts_client_uuid IS NOT NULL THEN 'Yes'\n" +
            "\tELSE offered.display END )AS offeredIns, \n" +
            "\t(CASE WHEN hie.hts_client_uuid IS NOT NULL THEN 'Yes' \n" +
            "\tELSE accepted.display END) as acceptedIns\n" +
            "FROM hts_client hc\n" +
            "LEFT JOIN hts_index_elicitation hie ON hie.hts_client_uuid = hc.uuid\n" +
            "LEFT JOIN base_application_codeset offered ON offered.id = CAST(CASE WHEN hc.index_notification_services_elicitation->>'offeredIns'='' THEN NULL ELSE hc.index_notification_services_elicitation->>'offeredIns' END AS BIGINT)\n" +
            "LEFT JOIN base_application_codeset accepted ON accepted.id = CAST(CASE WHEN hc.index_notification_services_elicitation->>'acceptedIns'='' THEN NULL ELSE hc.index_notification_services_elicitation->>'acceptedIns' END AS BIGINT)\n" +
            "LEFT JOIN (\n" +
            "SELECT * FROM (SELECT COUNT(uuid) AS NoOfPartnersElicited, hie.hts_client_uuid, hie.hts_client_uuid hts_client_uuid2023  \n" +
            "FROM hts_index_elicitation hie WHERE archived=0 GROUP BY hie.hts_client_uuid) partner\n" +
            ")partner ON partner.hts_client_uuid=hc.uuid\n" +
            "),\n" +
            "strat AS (SELECT DISTINCT hc.client_code AS strat_client_code, bac.display AS entryPoint, \n" +
            "\t\t  modality_code.display AS modality FROM hts_risk_stratification hrs \n" +
            "\t\t  LEFT JOIN hts_client hc ON hc.risk_stratification_code=hrs.code\n" +
   ...
   /* @Query(value = "WITH bio_data AS (SELECT DISTINCT (p.uuid) AS personUuid,p.hospital_number AS hospitalNumber, h.unique_id as uniqueId,\n" +
            ...
            ") dt where rowNum=1),\n"+
            "case_manager AS (SELECT DISTINCT ON (cmp.person_uuid)person_uuid AS caseperson, cmp.case_manager_id, \n" +
            " CONCAT(cm.first_name, ' ', cm.last_name) AS caseManager FROM (SELECT person_uuid, case_manager_id,\n" +
           ...
           "FROM bio_data bd\n" +
            ...
            , nativeQuery = true)
    List<RadetReportDto> getRadetData(
            Long facilityId,
            LocalDate start,
            LocalDate end,
            LocalDate previous,
            LocalDate previousPrevious,
            LocalDate dateOfStartOfCurrentQuarter,
            LocalDate viralLoadGracePeriodDate);*/
    ...
    @Query(value = "SELECT lo.uuid, lo.patient_uuid AS personUuid, lo.order_date AS orderDate, lo.archived, " +
            ...
            "INNER JOIN base_organisation_unit_identifier boui ON boui.organisation_unit_id=lo.facility_id AND boui.name='DATIM_ID' \n" +
    ...
    @Query(value = "SELECT lt.uuid, llt.lab_test_name AS labTestName, lltg.group_name AS groupName, \n" +
            ...
            "INNER JOIN base_organisation_unit_identifier boui ON boui.organisation_unit_id=lt.facility_id AND boui.name='DATIM_ID' \n" +
    ...
    @Query(value = "SELECT lr.uuid, lr.date_assayed AS dateAssayed, lr.date_result_reported AS dateResultReported, llt. lab_test_name AS labTestName, \n" +
            ...
            "INNER JOIN laboratory_labtest llt ON llt.id=lt.lab_test_id " +
            "INNER JOIN base_organisation_unit_identifier boui ON boui.organisation_unit_id=lr.facility_id AND boui.name='DATIM_ID' \n" +
    ...
    @Query(value = "SELECT pharmacy.*, boui.code AS datimId FROM (SELECT DISTINCT pharmacy.person_uuid AS personUuid, pharmacy.uuid, pharmacy.visit_date AS visitDate,\n" +
            ...
            "INNER JOIN hiv_regimen_type hrl ON hrl.id=hr.regimen_type_id " +
            ...
            "\t\t\tINNER JOIN base_organisation_unit_identifier boui ON boui.organisation_unit_id=phar.facility_id AND boui.name='DATIM_ID') boui ON \n" +
    ...
    @Query(value = "SELECT b.id AS uuid, b.person_uuid AS personUuid, b.template_type AS templateType,\n" +
            ...
            "INNER JOIN base_organisation_unit_identifier boui ON boui.organisation_unit_id=b.facility_id AND boui.name='DATIM_ID' " +
