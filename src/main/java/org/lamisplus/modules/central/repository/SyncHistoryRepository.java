...
public interface SyncHistoryRepository  extends JpaRepository<SyncHistory, Long> {
    ...
    @Query(value = "SELECT * FROM sync_history WHERE date_last_sync=(SELECT MAX(date_last_sync) from sync_history WHERE organisation_unit_id=?1) LIMIT 1", nativeQuery = true)
